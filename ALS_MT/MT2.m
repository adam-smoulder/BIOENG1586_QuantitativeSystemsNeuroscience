...
for i=1:length(coherence)
    ...
    for j=1:FRmax
        ...
        if ~isempty(LN_RC_Index) % preferred
...
% plot right neuron ROC
figure(5)
subplot(2,1,1)
set(gca,'FontSize',14)
hold on
title('ROC Curve, Right Neuron')
for i=1:length(coherence)
    plot(squeeze(percentAbove_RN_LC(1:FRmax,i)), squeeze(percentAbove_RN_RC(1:FRmax,i)), 'LineWidth', 3);
    accuracyNeuro2(i,1) = (-1)*trapz(squeeze(percentAbove_RN_LC(1:FRmax,i)), squeeze(percentAbove_RN_RC(1:FRmax,i)));
end
...
% plot Left Neuron ROC
subplot(2,1,2)
set(gca,'FontSize',14)
hold on
title('ROC Curve, Left Neuron')
for i=1:length(coherence)
    plot(squeeze(percentAbove_LN_RC(1:FRmax,i)), squeeze(percentAbove_LN_LC(1:FRmax,i)), 'LineWidth', 3);
    accuracyNeuro2(i,2) = (-1)*trapz(squeeze(percentAbove_LN_RC(1:FRmax,i)), squeeze(percentAbove_LN_LC(1:FRmax,i)));
end
legend('0','4','8','16','32','64')
axis([0,1,0,1])
